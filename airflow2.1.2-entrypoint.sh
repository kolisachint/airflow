...
if [ "$AIRFLOW__CORE__EXECUTOR" != "SequentialExecutor" ]; then
  ...
  if [ -z "$AIRFLOW__CORE__SQL_ALCHEMY_CONN" ]; then
    ...
    AIRFLOW__CORE__SQL_ALCHEMY_CONN="mysql+mysqlconnector://${MYSQL_USER}:${MYSQL_PASSWORD}@${MYSQL_HOST}:${MYSQL_PORT}/${MYSQL_DB}${MYSQL_EXTRAS}"
#    export AIRFLOW__CORE__SQL_ALCHEMY_CONN
    ...
    if [ "$AIRFLOW__CORE__EXECUTOR" = "CeleryExecutor" ]; then
      AIRFLOW__CELERY__RESULT_BACKEND="mysql+mysqlconnector://${MYSQL_USER}:${MYSQL_PASSWORD}@${MYSQL_HOST}:${MYSQL_PORT}/${MYSQL_DB}${MYSQL_EXTRAS}"
